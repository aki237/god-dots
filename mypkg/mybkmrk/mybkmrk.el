...
(defun mybkmrk/is-in-project ()
  ...
  (setq maxlength (length default-directory))
  (setq nowlength (length default-directory))
  (loop for i in mybkmrks do
	(setq nowlength (length (cdr (split-string
				      (mybkmrk/replace-string default-directory "~" (mybkmrk/get-abs-path "~") 1)
				      (mybkmrk/replace-string i "~" (mybkmrk/get-abs-path "~") 1)))))
	(if (and (< nowlength maxlength) (> (length (split-string
						     (mybkmrk/replace-string default-directory "~" (mybkmrk/get-abs-path "~") 1)
						     (mybkmrk/replace-string i "~" (mybkmrk/get-abs-path "~") 1))) 1 ))
	    (progn
	      (setq maxlength nowlength)
...
(split-string "/home/aki237/.emacs/mylisp/Something" "/home/aki237/gospace/src/github.com/aki237/salt")
(provide 'mybkmrk)
;; Local Variables:
;; coding: utf-8
;; End:
